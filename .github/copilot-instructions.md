# Copilot Instructions for gedcom_matcher

## Project Context
- This repository is a Dart package named `gedcom_matcher`.
- Current maturity: scaffold/template stage. Keep changes minimal, explicit, and test-backed.
- Dart SDK constraint is defined in `pubspec.yaml`.
- FVM is configured (`.fvmrc`) with Flutter 3.38.5.

## Preferred Commands
Run commands from repository root.

- Install dependencies: `fvm dart pub get` (fallback: `dart pub get`)
- Static analysis: `fvm dart analyze` (fallback: `dart analyze`)
- Tests: `fvm dart test` (fallback: `dart test`)
- Format: `fvm dart format lib test example` (fallback: `dart format lib test example`)

## Code Organization
- Public API entrypoint: `lib/gedcom_matcher.dart`
- Internal implementation: `lib/src/`
- Tests: `test/`
- Usage examples: `example/`

## API and Design Rules
- Export public symbols through `lib/gedcom_matcher.dart`; avoid exposing internal files directly.
- Keep internal implementation details in `lib/src/`.
- Prefer small, focused classes/functions with clear naming.
- Do not add dependencies unless they are necessary and justified.

## Testing Expectations
- Every new public API should have tests in `test/`.
- Prefer behavior-oriented tests with clear test names.
- Keep tests deterministic (no network, no clock dependence without injection).

## Style and Quality
- Respect `analysis_options.yaml` (`package:lints/recommended.yaml`).
- Keep code formatted with `dart format`.
- Update `README.md` and `CHANGELOG.md` for user-visible functionality changes.

## Common Pitfalls
- Breaking package API by importing/exporting files from `lib/src/` in client code.
- Leaving placeholder template symbols (`Awesome`) when introducing real features.
- Adding logic without corresponding tests.
